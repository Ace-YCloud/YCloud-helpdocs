...
* event type：设置：这个上报行为的触发条件。具体下面有介绍
...
<table><thead><tr><th width="170.33203125">Event type/事件类型</th><th>解释</th><th>适合哪些人</th></tr></thead><tbody><tr><td>为联系人打tag</td><td>您设置一个规则：当给某个wa用户 打tag=“ycloud”时，就视为：这个用户完成了一次广告转化。<br>-------------------<br>之后，<br>您给这个联系人打tag="ycloud"时，yc就认为：这个客户被转化了。会去<br></td><td><ol><li>广告转化数据不多，如：每天不超过50个；<br>（更多转化，还是建议走自定义事件。打tag毕竟是人工处理，可能搞错、忙不过来）</li></ol></td></tr><tr><td>自定义事件/API</td><td>通过第三方事件或您的系统对接API 来和YCloud对接，并传递"某客户被转化"。</td><td><p>符合任意一条</p><ol><li>您拥有技术开发的能力，可以对接YCloud的api。</li><li>您使用的平台，已和YCloud完成对接，如:Shopify。</li></ol></td></tr><tr><td>Whatsapp对话关键词(keyword)</td><td>设置1个或多个关键词。<br>当顾客在wa聊天发出的消息，包含这个关键词时，就视为转化。<br></td><td>适合：筛选高意愿度的lead时使用。<br><br>但注意，关键词有一些风险。<br>如：询问商品的支付方式，就视为高质量线索。<br>关键词=payment<br>但顾客询问时，<br>- 对话1：这个商品如何支付？<br>- 对话2：这个商品太烂了，我才不会支付。<br>显然，对话2的画像不是您需要的客户。</td></tr></tbody></table>
...
规则设置完毕后，当某个客户近期点击过广告，并完成了转化行为后：

请您的员工，在contact-找到这个客户，为他添加tag（您在track events设置的tag）。
...
2. 【触发上报】当客户在您的业务系统发生转化时，您调用YCloud的API来通知YCloud。

{% hint style="info" %}
**调用的api接口文档：**[custom event-send an event](https://docs.ycloud.com/reference/custom_events-send-event)。

当您调用接口返回200，说明YCloud已成功收到您的事件。<br>

YCloud会根据您上报的API数据（结合您在trackevents确定的规则、客户近期点击过的clid等参数），自动上报给TikTok广告平台。
{% endhint %}
...
1. Event-type 选择：**system events - keywords；**
2. 您需要设置一个特殊的标签，负责定义客户的转化行为。 &#x20;
   1.

       <figure><img src="../../.gitbook/assets/chooose-eventtype-keywordvalue-t1 (1).png" alt=""><figcaption></figcaption></figure>
   2. 点击+ keyword，来输入关键词。(可输入多个)
3.  选择要上报给TikTok的事件，打开开关，添加。&#x20;

    <figure><img src="../../.gitbook/assets/chooose-eventtype-keywordvalue1.png" alt=""><figcaption></figcaption></figure>
4. 保存后效果

<figure><img src="../../.gitbook/assets/trackevents-list-keywords-t1.png" alt=""><figcaption></figcaption></figure>
...
当某个客户近期点击过广告，，并在wa发出的消息，包含上述任一关键词时，YCloud会认为该客户完成转化，并自动上报给TikTok广告平台。
